...
uint8_t W5100Class::init(void)
{
	...
	// reset its SPI1 state when CS goes high (inactive).  Communication
	...
	// SPI1 well with this chip.  It appears to be very resilient,
	...
	// that's heard other SPI1 communication if its chip select
	// pin wasn't high when a SD card or other SPI1 chip was used.
	} else {
...
uint16_t W5100Class::read(uint16_t addr, uint8_t *buf, uint16_t len)
{
	...
	if (chip == 51) {
		for (uint16_t i=0; i < len; i++) {
			...
			cmd[2] = addr & 0xFF;
			cmd[3] = 0;
			SPI1.transfer(cmd, 4); // TODO: why doesn't this work?
			buf[i] = cmd[3];
			addr++;
			#endif
			resetSS();
